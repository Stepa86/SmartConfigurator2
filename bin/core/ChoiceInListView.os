...
Процедура ОбработатьВыборСтроки()
	...
	ДополнительныеПараметры.Вставить("ТекущаяСтрока", ПолеСписка.ТекущаяСтрока);
	ЭтаФорма.Закрыть();
...
Функция ВыбратьИзСписка(Данные, ПолеДляФильтрации, ПолеНомераСтроки, ЗаголовокОкна = "", Параметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если ДополнительныеПараметры.Свойство("ТекущаяСтрока") Тогда
			ПолеСписка.ТекущаяСтрока = ДополнительныеПараметры.ТекущаяСтрока;
			ПолеСписка.ВыделенныеСтроки.Добавить(ДополнительныеПараметры.ТекущаяСтрока);
		КонецЕсли;
