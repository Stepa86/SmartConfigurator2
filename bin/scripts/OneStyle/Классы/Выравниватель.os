...
Функция СформироватьОтступПеренесеннойСтроке(Знач пТекСтрока,
	...
	ИначеЕсли ЭтоПереносСтроки(пПредСтрока, пТекСтрока)
		И КодНачинаетсяСтрокой(пПредСтрока) Тогда
	...
	ИначеЕсли КодЗаканчиваетсяНа(пПредСтрока, "(")
		ИЛИ КодЗаканчиваетсяНа(пПредСтрока, "=")
...
Функция ЭтоПереносСтроки(Знач пПредСтрока, Знач пТекСтрока)
	
	Если Не КодНачинаетсяНа(пТекСтрока, "|") Тогда
		
		Возврат Ложь;
	
	КонецЕсли;
	
	предСтрокаБезКомментариев = СокрЛП(КодБезКомментариев(пПредСтрока));
	
	количествоКавычек = СтрЧислоВхождений(предСтрокаБезКомментариев, """");
	
	Если КодНачинаетсяНа(предСтрокаБезКомментариев, "|") Тогда
...
Функция КодНачинаетсяСтрокой(Знач пКод)

	кодБезКомментариев = СокрЛП(КодБезКомментариев(пКод));
	
	Возврат СтрНачинаетсяС(кодБезКомментариев, """")
		ИЛИ СтрНачинаетсяС(кодБезКомментариев, "|");
